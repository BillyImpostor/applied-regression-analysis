...
#Dimodelkan dengan Regresi Linear Sederhana
library(lmtest)
library(stats)
library(car)
...
plot(data$x,data$y)
lines(data$x,fitted(model),col="red")

#Ketika dimodelkan dengan RLS diperoleh nilai Koefisien determinasi sebesar 8.78%
#Kemudian secara simultan, variabel bebas berpengaruh terhadap variabel terikat
#Secara parsial pun, variabel bebas berpengaruh terhadap variabel terikat
...
#Otokorelasi
acf(res1,38)
dwtest(model,data=data)

#Normalitas
shapiro.test(res1)

#Secara garis besar regresi linear sederhana TIDAK TEPAT untuk digunakan dalam pemodelan ini.

#Selanjutnya coba dimodelkan dengan menggunakan model-model polinomial

#Polinomial derajat 1 akan sama dengan regresi linear sederhana

#Polinomial derajat 2 (fungsi kuadratik)
y=data$y
x=data$x
xkuadrat=(data$x)^2
model2<-lm(y~x+xkuadrat)
summary(model2)

prediksi<-predict(model2)
plot(data$x,data$y)
lines(data$x,fitted(model2),col="red") #Koefisien Determinasi adalah 14.4%

#Polinomial derajat 3 (fungsi kubik)
y=data$y
x=data$x
xkuadrat=(data$x)^2
xkubik=(data$x)^3
model3<-lm(y~x+xkuadrat+xkubik)
summary(model3)

prediksi<-predict(model3)
plot(data$x,data$y)
lines(data$x,fitted(model3),col="red") #Koefisien Determinasi adalah 33.11%
